...
for file_name in paths:
  ...
  for each in sorted(tuning, key=int):
    with open(basename + '/' + file_name[:-4] + "_tuning_fold_" + str(int(each)).zfill(x), 'a') as f:
      nu = ''
      for a in tuning[each]:
  ...
  for each in sorted(validation, key=int):
    with open(basename + '/' + file_name[:-4] + "_validation_fold_" + str(int(each)).zfill(x), 'a') as f:
      nu = ''
      for a in validation[each]:
